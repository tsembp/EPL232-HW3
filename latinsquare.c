...
int main(int argc, char *argv[])
{
    ...
    if (argc != 2)
    { // check if arguments contain 2 inputs, ./latinsquare and input file name
...
void readLatinSquare(const char *filename, short square[N][N], int *size)
{
    ...
    for (int i = 0; i < *size; i++)
    {
        ...
        {
            if (fscanf(fp, "%hd", &square[i][j]) != 1)
            { // check fscanf return code for error
                printf("Error reading Latin Square values.\n");
                fclose(fp);
                exit(EXIT_FAILURE); // close file and exit
            }
            else if (square[i][j] > *size || square[i][j] < -(*size))
            { // check if values of latin square are in [1, size]
...
void displayLatinSquare(short square[N][N], int size)
{
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        {
            if (square[i][j] < 0)
            {                                       // check if value at i,j is negative (pre-given value)
                printf("| (%-1d) ", -square[i][j]); // print number in parenthesis in space of two
            }
            else
            { // print other values (given by user)
...
int checkUserInput(int i, int j, int val, int size, short square[N][N])
{
    ...
    for (int row = 0; row < size; row++)
    {
        ...
        {
            if (val != 0 && abs(square[row][col]) == val)
